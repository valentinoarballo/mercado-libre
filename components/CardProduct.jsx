...
function CardProduct({ item, itemKey }) {
  ...
  return (
    <a
      href={`/search/${item.id}`}
      key={itemKey}
      className='bg-white m-3 rounded-lg px-4 pb-4 flex flex-col justify-evenly h-auto min-h-[375px] xl:w-[19rem] lg:w-[25rem] w-[30rem] hover:cursor-pointer shadow-lg hover:shadow-xl hover:text-blue-400'
    >

      <div className="flex wrap flex-wrap w-full justify-center">
        <img className='max-h-[13rem] max-w-[19rem] h-auto w-auto' src={item.thumbnail} alt={item.thumbnail_id} />
      </div>
      <div>
        <p className='text-lg w-full'>{item.title.slice(0, 25)}...</p>
